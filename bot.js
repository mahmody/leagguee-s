...
client.on('message', function(message) {
    if (message.channel.type === "dm") {
        ...
        client.channels.get("595662156457050123").send({ embed: iiMo });
    }
});
...
client.on('message', message => {
        if (message.content.toLowerCase() === prefix + "invitedemon") {
            if(!message.channel.guild) return;
...
var config = {
  events: [
    ...
    {type: "CHANNEL_DELETE", logType: "CHANNEL_DELETE", limit: 4, delay: 4000},
    {type: "GUILD_MEMBER_REMOVE", logType: "MEMBER_KICK", limit: 4, delay: 4000},
    {type: "GUILD_BAN_ADD", logType: "MEMBER_BAN_ADD", limit: 4, delay: 4000}
  ]
}
...
gmemb = guild.members.size;
groles = guild.roles.map(r=> {return r.name});
  let channel = client.channels.get('594907944789737474')
  if(!channel) return;
const e = new Discord.RichEmbed()
.setColor('#36393e')
